...
produce_config_by_process_name()
{
    ...
    then
        ...
        # 配置/data/projects/kaiwu-fwk/thirdparty/model_pool_go/config下面的gpu.iplist
        learner_ip_list=(`echo $learner_ip_list_str | tr ',' ' '`)
        gpu_configue_file=/data/projects/kaiwu-fwk/thirdparty/model_pool_go/config/gpu.iplist
        cat /dev/null > $gpu_configue_file
    ...
    then
        ...
        # 换行, 规避和原来的内容覆盖
        echo -e "\nmodelpool_remote_addrs = $host_ip:10014" >> $configure_file
        echo "ip_address = $host_ip" >> $configure_file

        # 配置/data/projects/kaiwu-fwk/tools下的run_mulit_learner_by_openmpirun.sh里的Nodelist和Num_process
        openmpi_congiure_file=/data/projects/kaiwu-fwk/tools/run_mulit_learner_by_openmpirun.sh
