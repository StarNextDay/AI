...
# 进程启动前请注意conf下的配置文件, 主要是configue.ini, aisrv.ini, learner.ini, actor.ini里参数配置
# app conf和algo conf需要按照实际情况配置
sh tools/start.sh 

# 步骤3, 确认进程启动成功, 启动自带的client, 注意参数需要正确 python app/sgame/tools/start_multi_game.py 需要运行的轮数 配置文件
python3 app/sgame/tools/start_multi_game.py 1 /data/projects/kaiwu-fwk/conf/client.ini
