...
then
    ...
    python3 $Work_dir/framework/server/python/learner/learner.py --conf=$Work_dir/conf/learner.ini >$Work_dir/log/learner/horovodrun.log 2>&1 &
else
    # 开发测试环境使用, 非后台进程, 打印日志到屏幕
    horovodrun -np ${Num_process} -H ${Nodelist} --ssh-port 36000 --ssh-identity-file $Ssh_key \
    python3 $Work_dir/framework/server/python/learner/learner.py --conf=$Work_dir/conf/learner.ini 
fi
